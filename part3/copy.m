...
%% Static obstacles
lbox = 0.2*2; % length of the cube
box_center = [0.6 0.46 0.26]; % (metric) world coordinates of the box center
aObs = collisionBox(lbox,lbox,lbox);  
aObs.Pose = trvec2tform(box_center); 

box_center2 = [0.2 0.46 0.75];
a1bs = collisionBox(lbox,lbox,lbox);
a1bs.Pose = trvec2tform(box_center2);

box_center3 = [-0.2 0.46 0.26];
a2bs = collisionBox(lbox,lbox,lbox);
a2bs.Pose = trvec2tform(box_center3);


% Set static obstacles
world = {aObs,a1bs,a2bs}; % try adding more static obstacles aObs to dObs or create your own
...
cube_metric2 = [box_center2-lbox/2;
                lbox,lbox,lbox]; 
cube_metric3 = [box_center3-lbox/2;
                lbox,lbox,lbox]; 
...
cube_voxel3 = [ceil((cube_metric3(1, :)-Env_size(1,:))./voxel_size); ...
    ceil((cube_metric3(1, :) + cube_metric3(2, :)-Env_size(1,:))./voxel_size)];
...
[xc3, yc3, zc3] = meshgrid(cube_voxel3(1, 1):cube_voxel3(2, 1), cube_voxel3(1, 2):cube_voxel3(2, 2), cube_voxel3(1, 3):cube_voxel3(2, 3));
...
binary_world(sub2ind([Env_size(2, 1) / voxel_size(1), Env_size(2, 2) / voxel_size(2), Env_size(2, 3) / voxel_size(3)], xc3, yc3, zc3)) = 1;
