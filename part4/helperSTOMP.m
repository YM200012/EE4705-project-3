...
RAR_time = [];
mat = cell(1,numJoints);

for i =1:numJoints
    mat{i} = robot_struct.Bodies{1,i}.Joint.JointToParentTransform;
end

[~, Qtheta] = stompTrajCost(robot_struct, theta, R, voxel_world,mat);
QthetaOld = 0;
...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    for k = 1:nPaths
        [S,Q] = stompTrajCost(robot_struct, theta_samples{k}, R, voxel_world,mat);
        Stheta(k,:) = S;
    end
